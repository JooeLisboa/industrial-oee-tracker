...
def register_crud(model, path, roles):
    ...
    def list_items(model=model):
        return jsonify([serialize(x) for x in model.query.all()])
...
def plan_list():
    ...
    return jsonify([serialize(x) for x in q.order_by(DailyPlanItem.priority.asc()).all()])
...
def run_open():
    ...
    if data.get('run_date'):
        ...
        end_at = datetime.combine(run_date + (timedelta(days=1) if shift.crosses_midnight else timedelta()), shift.end_time)
    ...
    if not run:
        ...
        if not op:
            ...
            db.session.add(op)
            db.session.flush()
        run = ProductionRun(run_date=run_date, shift_id=shift_id, machine_id=machine_id, op_id=op.id, start_at=start_at, end_at=end_at, created_by=uid)
        db.session.add(run)
        db.session.flush()
...
def downtime_start():
    ...
    ev = DowntimeEvent(run_id=run.id, reason_id=reason.id, start_at=_now(), created_by=uid, notes=data.get('notes'))
    db.session.add(ev)
    db.session.flush()
    state = _ensure_machine_state(run.machine_id)
...
def run_close():
    ...
    run.closed_by = uid
    if _open_segment(run.id):
        _open_segment(run.id).end_at = _now()
    if _open_downtime(run.id):
        _open_downtime(run.id).end_at = _now()
    state = _ensure_machine_state(run.machine_id)
    state.status = MachineStatus.IDLE
    state.current_segment_id = None
    state.current_downtime_id = None
...
def _audit(entity_type, entity_id, before, after, reason):
    uid = int(get_jwt_identity())
    item = AuditLog(entity_type=entity_type, entity_id=entity_id, action='UPDATE', before_json=before, after_json=after, reason=reason, changed_by=uid)
    db.session.add(item)
...
def audit_list():
    ...
    return jsonify([serialize(x) for x in q.order_by(AuditLog.id.desc()).all()])
...
def status_overview():
    ...
    for line in Line.query.all():
        machines = []
        for m in Machine.query.filter_by(line_id=line.id).all():
            st = _ensure_machine_state(m.id)
            machines.append({'machine': serialize(m), 'state': serialize(st)})
...
def report_machine(machine_id):
    ...
    runs = ProductionRun.query.filter(ProductionRun.machine_id == machine_id, ProductionRun.start_at >= from_dt, ProductionRun.start_at <= to_dt).all()
    return {'machine_id': machine_id, 'items': [calculate_oee(r.id) for r in runs]}
...
def report_monthly():
    ...
    runs = ProductionRun.query.filter(db.extract('year', ProductionRun.run_date) == year, db.extract('month', ProductionRun.run_date) == month).all()
    items = [calculate_oee(r.id) for r in runs]
...
def settings_public():
    s = Setting.query.first() or Setting(company_name='OEE Line Monitor')
    if not s.id:
        db.session.add(s)
        db.session.commit()
    return serialize(s)
