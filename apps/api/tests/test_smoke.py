...
def test_open_run_and_start_segment(client, auth_header):
    ...
    seg = client.post('/api/segments/start', headers=auth_header, json={'run_id': run.get_json()['id'], 'product_id': 1})
    assert seg.status_code == 201


def test_downtime_updates_machine_state(client, auth_header):
    run = client.post('/api/runs/open', headers=auth_header, json={'machine_id': 1, 'shift_id': 1, 'op_code': 'OP-2'}).get_json()
